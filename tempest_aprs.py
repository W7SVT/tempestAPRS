    ...
    try:
        ...
        if observation.get("type") == "obs_st":
            ...
            if obs_data and len(obs_data[0]) >= 17:
                ...
                local_time = datetime.fromtimestamp(time_epoch).strftime("%Y-%m-%d %H:%M:%S")
                ...
                aprs_message = f">WX Beacon: {local_time}, Wind: {wind_speed_mph}mph {wind_dir}°, Temp: {air_temp_fahrenheit}°F, Pressure: {station_pressure_mb}mb"
